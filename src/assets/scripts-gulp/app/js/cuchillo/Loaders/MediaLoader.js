class MediaLoader extends CustomLoader{
    ...
    getMedia(__all = false) {
        ...
        C.Selector.forEach("[" + TYPE_MEDIA + "]", function(el, i) {
            ...
            const HAS_EFFECT = !el.hasAttribute("data-no-effect");
            const SHOW_AT_END = !el.hasAttribute("data-no-show");
        ...
        C.Selector.forEach("[data-aspect-ratio]", function(el, i) {
            //if(el.parentNode.classList.contains("media-holder")) {
                let w = Number(el.getAttribute("width")) || Number(el.getAttribute("data-width"));
                let h = Number(el.getAttribute("height")) || Number(el.getAttribute("data-height"));
